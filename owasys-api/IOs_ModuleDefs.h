 ...
 * @param wEdge Edge to interrupt: 0 for falling edge, 1 for rising edge, 2 for both, 3 high level, 4 low level
...
/**
 * @brief Configures the polling time to read IOs value.
 *
 * For all digital inputs, when configuring level interrupts, they are
 * polled every PollTime time to set decide if value is HIGH or LOW. 
 * Logic filter implemented:
 * 1. was LOW and is LOW
 * 2. was LOW and is HIGH
 * 3. was HIGH and is HIGH
 * 4. was HIGH and is LOW
 * 
 * A change to states 1 o 3 leads to a change in the (queryable) state of the IO.
 * A change to states 2 o 4 marks and edge and does not yet lead to a change.
 *
 * @param PollTime Time in milisecons (5 to 50, default 10) for IO polling
 * 
 */
   int DIGIO_SetLevelInputPollTime( u_int16_t PollTime);
...
#ifdef OWA344
   /**
 * @brief Enable/Disable current meter.
 *
 * @param wValue: 0: disable current meter, 1: enable current meter
 * @return NO_ERROR if success. Specific error number if fails.
 */
   int DIGIO_Set_C_METER_ON( unsigned char wValue );
#endif
#if defined OWA344 || defined OWA5X
   /**
 * @brief Enable/Disable USB_0.
 *
 * @param wValue: 0: disable USB_0, 1: enable USB_0
 * @return NO_ERROR if success. Specific error number if fails.
 */
   int DIGIO_Set_USB_0_ON( unsigned char wValue );
#endif
