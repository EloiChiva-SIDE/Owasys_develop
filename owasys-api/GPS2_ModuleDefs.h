...
//-----------------------------------------------------------------------//
   ...
   #define FLAG_RTK_FIX    0x0100  /**< RTK_FIX solution got from GGA */
   #define FLAG_RTK_FLOAT  0x0200  /**< RTK_FLOAT solution got from GGA */

   #if defined(OWA3X) || defined(OWA4X) || defined(OWA5X)
    #define ALL_FIX_FLAGS (FLAG_NF | FLAG_DR | FLAG_G2 | FLAG_G3 | FLAG_D2 | FLAG_D3 | FLAG_RK | FLAG_TT)
   #elif defined(OWA344)
    #define ALL_FIX_FLAGS (FLAG_NF | FLAG_DR | FLAG_G2 | FLAG_G3 | FLAG_D2 | FLAG_D3 | FLAG_RK)
   #endif
   ...
   #define STATUS_RK          6  /**< RK Combined GPS + dead reckoning solution from pubx,00 */
   #define STATUS_TT          7  /**< TT Time only solution from pubx,00 */
   #define STATUS_RTK_FIX     8  /**< RTK fixed got from GGA */
   #define STATUS_RTK_FLOAT   9  /**< RTK float got from GGA */
...
//-----------------------------------------------------------------------//
  ...
  */
	...
	typedef struct {    // M8P / F9P high precission information structure
      double         relPosN;       /**< North component of relative position vector */
      double         relPosE;       /**< East component of relative position vector */
      double         relPosD;       /**< Down component of relative position vector */
      double         accN;          /**< Accuracy of relative position North component */
      double         accE;          /**< Accuracy of relative position East component */
      double         accD;          /**< Accuracy of relative position Down component */
      unsigned char  diffSoln;      /**< 1=differential corrections were applied */
      unsigned char  relPosValid;   /**< 1=relative position components and accuracies are valid */
      unsigned char  carrSoln;      /**< Carrier phase range solution status: 0=no carrier phase 1=carrier phase with floating ambiguities 2=carrier phase with fixed ambiguities*/
    }tNAV_RELPOSNED; 

    typedef struct {    // M8P / F9P high precission information structure
      time_t         time;          /**< Timestamp of last received message */
      unsigned char  crcFailed;     /**< Status of CRC parsing on the GPS module,1 if failed, 0 if ok */
      unsigned char  msgUsed;       /**< 0=Not known 1=not used 2=RTCM message used successfully */    
      int            subType;       /**< RTCM message subtype */
      int            refStation;    /**< Reference station ID */
      int            msgType;       /**< RTCM message type */
    }tRXM_RTCM;
...
/**
 * @brief Return the "Relative Positioning Information in NED frame" message from the GPS receiver M8P or ZED-F9P.
 * 
 * @param *pData Contains the tNAV_RELPOSNED structure values.
 * @return NO_ERROR if success. Specific error number if fails.
 */
   int GPS_GetRELPOSNED( tNAV_RELPOSNED *pData ); // ****

/**
 * @brief Return information about the last "RTCM" message sent to the receiver.
 *
 * @param *pData Contains the tRXM_RTCM structure values.
 * @return NO_ERROR if success. Specific error number if fails.
 */
   int GPS_GetRTCM( tRXM_RTCM *pData ); // ****
